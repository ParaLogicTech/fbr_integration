frappe.ui.form.on("Sales Invoice", {
	...
	refresh_fbr_di_values(frm) {
		return frappe.call({
			method: "fbr_integration.fbr_integration.fbr_di_integration.refresh_fbr_di_values",
			args: {
				invoice: frm.doc
			},
			freeze: 1,
			freeze_message: __("Recalculating FBR Digital Invoicing Values"),
			callback: (r) => {
				if (r.message) {
	...
	sync_fbr_di_invoice(frm) {
		return frappe.call({
			...
			freeze: 1,
			freeze_message: __("Posting to FBR Digital Invoicing Service"),
			callback: (r) => {
				if (!r.exc && r.message) {
					frm.reload_doc();
				}
	...
	sync_fbr_pos_invoice(frm) {
		return frappe.call({
			...
			freeze: 1,
			freeze_message: __("Posting to FBR POS Service"),
			callback: (r) => {
				if (!r.exc && r.message) {
					frm.reload_doc();
				}
